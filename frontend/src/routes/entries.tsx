...
import {
  ...
  Minus, IndianRupee, Loader2, Save, Download, Share2,
  CheckCircle2, Clock,
} from "lucide-react";
...
function exportSlotToExcel(rows: Entry[], slotLabel: string) {
  if (rows.length === 0) {
...
function shareSlotSummary(
  ...
  const totalCases = rows.reduce((a, e) => a + e.totalCases, 0);
  const totalAmount = rows.reduce((a, e) => a + e.totalAmount, 0);
  const text =
    `Entries Summary — ${slotLabel}\n` +
    `Entries: ${rows.length} (Draft: ${draftCount}, Submitted: ${submittedCount})\n` +
    `Total Cases: ${totalCases}\n` +
    `Total Amount: ${formatINR(totalAmount)}\n` +
    `Avg/Day: ${avgCases.toFixed(1)} cases, ${formatINR(avgAmount)}`;
    ...
    <CollectorLayout>
     ...
     {/* Slot summary — only shown when a slot is active */}
      {activeSlot && (() => {
        ...
        const totalAmount = filtered.reduce((a, e) => a + e.totalAmount, 0);
        const draftCount = filtered.filter((e) => e.status === "draft").length;
        const submittedCount = filtered.filter((e) => e.status === "submitted").length;
        const workingDays = new Set(filtered.map((e) => e.date)).size;
        const avgCases = workingDays > 0 ? totalCases / workingDays : 0;
        const avgAmount = workingDays > 0 ? totalAmount / workingDays : 0;

        return (
          <div className="mb-4 rounded-2xl border border-primary/30 bg-primary-soft p-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <div className="text-2xl font-black text-primary">{totalCases}</div>
                <div className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                  Total Cases
                </div>
              </div>
              <div>
                <div className="text-2xl font-black text-primary">{formatINR(totalAmount)}</div>
                <div className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                  Total Amount
                </div>
              </div>
            ...
            {/* Draft vs Submitted */}
            <div className="mt-3 flex gap-4 border-t border-primary/20 pt-3">
              <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                <Clock className="h-3.5 w-3.5" /> Draft: {draftCount}
              </div>
              <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                <CheckCircle2 className="h-3.5 w-3.5" /> Submitted: {submittedCount}
              </div>
            </div>

            {/* Average per working day */}
            <div className="mt-2 text-xs text-muted-foreground">
              Avg/Day ({workingDays} working {workingDays === 1 ? "day" : "days"}):{" "}
              <span className="font-semibold text-foreground">
                {avgCases.toFixed(1)} cases, {formatINR(avgAmount)}
              </span>
            </div>
            ...
            <div className="mt-3 flex gap-2">
              <button
                onClick={() => exportSlotToExcel(filtered, slotLabel)}
                className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-primary/40 bg-card px-3 py-2 text-xs font-semibold text-primary hover:bg-primary/10"
              >
                <Download className="h-3.5 w-3.5" /> Export
              </button>
              <button
                onClick={() =>
                  shareSlotSummary(filtered, slotLabel, draftCount, submittedCount, avgCases, avgAmount)
                }
                className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-primary/40 bg-card px-3 py-2 text-xs font-semibold text-primary hover:bg-primary/10"
              >
